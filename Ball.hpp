...
inline Ball::Ball(Player &lastTouch) { this->lastTouch = &lastTouch; }

inline void Ball::Move() {
  position =
      Vector2Add(position, Vector2Scale(Vector2Normalize(direction), speed));
}

inline void Ball::Accelerate(float acceleration) { speed += acceleration; }
...
inline void Ball::CollideWithPlayer(float reflectTo, Player &player) {
  if (CheckCollisionCircleRec(position, radius, player.rect)) {
    ...
    // It does it by remaping the distance(can be negative) between the ball and
    // center of the player to a value from 0 to 1
    direction = Vector2Lerp(
        {reflectTo, -1}, {reflectTo, 1},
        Remap(position.y - player.getY() - (float)player.getHeight() / 2,
              -(float)player.getHeight() / 2, (float)player.getHeight() / 2, 0,
              1));

    // This calc is kinda wierd, so i'll be leaving these debug couts were
    // std::cout << "player height: " << player.getHeight() << '\n';
    // std::cout << "input: " << position.y - player.getY() -
    // player.getHeight()/2 << "\nStart: " << -player.getHeight()/2 - radius <<
    // "\nEnd: " << player.getHeight()/2 + radius<< '\n'; std::cout << "Remap: "
    // << Remap(position.y - player.getY() - player.getHeight()/2,
    // -player.getHeight()/2, player.getHeight()/2, 0, 1) << '\n';
    lastTouch = &player;
